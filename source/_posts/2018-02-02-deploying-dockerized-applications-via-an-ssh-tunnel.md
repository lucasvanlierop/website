...
## Why deploy Dockerized applications over over SSH instead of directly via the API?
...
However... not everyone (or their system administrator) wants to expose yet another port to the outside world.
...
*(If you're unsure how to set this up, tell me in the comments. I might write an article about it!).*
- Knowledge about writing [Docker Compose v3 files](https://docs.docker.com/compose/compose-file/)
...
There's one gotcha though: this only works on SSHv6.7+ while most older OSes are stuck on SSHv6. 
...
A easy way to do that is to poll the tunnel by executing a simple docker command like `docker version`.
...
*Note: if the connection cannot be established for some reason the loop will run forever.
